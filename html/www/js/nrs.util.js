...
(isNode ? client : NRS).onSiteBuildDone().then(() => {
    NRS = (function (NRS, $, undefined) {
        ...
        NRS.ardorToAccountPrefix = function (account) {
            const prefix = "ARDOR-"
            if (!account || account.substring(0, prefix.length).toUpperCase() !== prefix) {
                return account;
            }
            return NRS.getAccountMask() + account.substring(prefix.length);
        }
